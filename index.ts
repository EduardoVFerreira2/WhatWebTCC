...
function getListaContas(trazerConectadas: boolean | undefined) {
  const currentDateTime = new Date().toLocaleString();

  console.log(`[${currentDateTime}] ` + "Atualizando lista de contas");

  api
    .get(`WhatsAppConta/Lista/${trazerConectadas ? 1 : 0}`)
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close" && lastDisconnect) {
      ...
      if (shouldReconnect) {
        ...
        DestroyClient(conta_id);
        if (statusMessage.includes("QR refs attempts ended")) {
          enviarEventoApi(conta_id, EventType.TIME_OUT, {
            body: "Tentativas maximas de geração excedidas",
          });
        } else {
          enviarEventoApi(conta_id, EventType.DISCONNECTED, {
          ...
          if (isGroup) {
            if (
              ...
              try {
                ...
                console.error(
                  "Erro ao obter os participantes do grupo:",
                  error
                );
              }
            }
...
async function DestroyClient(conta_id: string) {
  ...
  try {
    ...
    console.log(
      `[${currentDateTime}] Cliente destruído para conta ${conta_id}`
    );
...
app.post("/send", async (req, res) => {
  ...
  try {
    ...
    if (!result?.[0].exists) {
      ...
      res
        .status(400)
        .send({ cod: 1, msg: "Número não registrado no WhatsApp" });
      return;
    }
    ...
    if (error instanceof Error) {
      console.error(
        `[${currentDateTime}] ` + "Erro ao enviar mensagem:",
        error
      );
      vRetorno.cod = 1;
